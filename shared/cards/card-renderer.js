(function (global) {
  ...
  function html(input, options={}) {
    const card=normalizeCard(input), mode=options.mode || 'collection';
    const longTitle=card.name.length > 23;
    const compact=card.rulesText.length > 112;
    ...
    return `<article class="systema-card systema-card--${escape(mode)} systema-card--type-${slug(card.type)} systema-card--rarity-${slug(card.rarity)} ${longTitle?'systema-card--long-title':''} ${compact?'systema-card--compact-rules':''} ${options.playable===false?'is-unplayable':''} ${options.selected?'is-selected':''}" data-systema-card data-card-id="${escape(card.id)}" data-card-template="${escape(global.SystemaCardLayout?.template || 'premium-v1')}" tabindex="${options.tabIndex ?? 0}" aria-label="${escape(`${card.name}, cost ${card.cost}, ${card.type}, ${card.rarity}. ${card.rulesText}`)}">
      ...
      <div class="systema-card__art ${art?'has-art':'no-art'}" style="--art-x:${card.artwork.focalX}%;--art-y:${card.artwork.focalY}%;--art-zoom:${card.artwork.zoom}">${art?`<img src="${escape(art)}" alt="" decoding="async">`:'<span aria-hidden="true"></span>'}</div>
      <section class="systema-card__rules"><p>${emphasizeRules(card.rulesText)}</p><div class="systema-card__keywords">${keywordsHtml(card)}</div>${compact&&mode!=='expanded'?'<span class="systema-card__inspect-hint">Inspect for full details</span>':''}</section>
  ...
  function fit(root) { if(!root) return; requestAnimationFrame(()=>{const rules=root.querySelector('.systema-card__rules'),text=rules?.querySelector('p'),keywords=rules?.querySelector('.systema-card__keywords'); const clipped=Boolean(rules&&(rules.scrollHeight>rules.clientHeight||text?.scrollHeight>text?.clientHeight||keywords?.scrollHeight>keywords?.clientHeight)); if(clipped){root.classList.add('systema-card--compact-rules');if(!rules.querySelector('.systema-card__inspect-hint')&&!root.classList.contains('systema-card--expanded'))rules.insertAdjacentHTML('beforeend','<span class="systema-card__inspect-hint">Inspect for full details</span>');}}); }
  ...
  function openPreview(input, options={}) { const card=normalizeCard(input), dialog=ensureDialog(); dialog._returnFocus=document.activeElement; renderInto(dialog.querySelector('.systema-card-dialog__content'),card,{...options,mode:'expanded'}); dialog.querySelector('.systema-card-dialog__glossary').innerHTML=`<p class="systema-card-dialog__full-rules"><b>Complete rules</b> ${emphasizeRules(card.rulesText)}</p>`+card.keywords.map(k=>{const d=global.SystemaCardKeywords.get(k);return d?`<p><b>${escape(d.label)}</b> ${escape(d.full)}</p>`:''}).join(''); dialog.showModal(); dialog.querySelector('button').focus(); return dialog; }
